...
_AGENT_PATH = Path(__file__).resolve().parent / "lingbot_map_agent.py"
...
class LingBotMapAgentClient:
    ...
    def __init__(
        ...
        *,
        ...
        ready_line = self._process.stdout.readline(timeout=start_timeout_s)
        if not ready_line:
            raise RuntimeError(
...
def build_camera_to_world(frame: "FrameObservation") -> list[float]:  # noqa: F821
    ...
    """
    from agentic_memory_nav.execution.isaacsim_adapter import _GO2_CAMERA_OFFSET_M
