#!/usr/bin/env python3
"""LingBot-Map perception sub-agent.

This module used to live at ``scripts/lingbot_map_worker.py`` and was launched as a
plain "worker". It is now a first-class perception **sub-agent** inside the
``agentic_memory_nav`` package:

* ``LingBotMapAgent``   -- the model runtime (one GCTStream instance) that ingests
  the head camera's RGB frames and emits camera pose, depth, confidence, intrinsics
  and a global point cloud. It is the "brain" of the mapping sub-agent.
* ``LingBotMapAgentClient`` -- the in-process client that the Isaac Sim main
  process uses. It spawns ``LingBotMapAgent`` as an isolated subprocess (the LingBot
  model must run in its own Python 3.10 / CUDA venv, separate from Isaac Sim's
  Python 3.12), feeds it one frame request at a time over stdio pipes, and decodes
  the reply. It is *also* the ``LingBotPredictor`` callable consumed by
  ``LingBotMapAdapter``, so it is the single dispatch entry-point for the mapping
  sub-agent.
* ``run_agent`` / ``main`` -- the standalone process entry-point. The client launches
  it with the LingBot venv's python; it loads the checkpoint, prints a ``ready``
  signal, then serves one JSON request per line until stdin closes.

Dispatch model
--------------
The Isaac Sim main process does not import or run the LingBot model itself. It asks a
``SubAgentDispatcher`` (see :mod:`agentic_memory_nav.agent.lingbot.client`) to start the
LingBot map sub-agent at simulation start and to guarantee it is alive (fail fast if it
is not). Every frame the dispatcher hands the RGB request to the agent and returns the
decoded mapping result. This keeps the heavy, version-locked LingBot runtime isolated
while exposing a uniform sub-agent interface to the rest of the pipeline.
"""
...
class LingBotMapAgent:
    """One GCTStream instance with the same streaming calls as LingBot's demo.

    This is the isolated runtime that actually loads the checkpoint and runs the
    model. It lives in the LingBot venv process; the Isaac main process only talks to
    it through :class:`LingBotMapAgentClient`.
    """
...
def run_agent(args: argparse.Namespace) -> int:
    """Serve one JSON request per line until stdin closes (the sub-agent process)."""
    runtime = LingBotMapAgent(args)
