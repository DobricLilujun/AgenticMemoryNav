...
configured vLLM endpoint) looks at each RGB frame and picks one of the six
standard discrete actions:

  turn_left | turn_right | move_forward | stop | look_up | look_down
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--move-step-m", type=float, default=0.25, help="meters advanced per move_forward"
    )
    parser.add_argument(
...
def _build_agent(
    config, instruction: str, max_look_count: int
) -> VLMDiscreteNavigationAgent:
...
def main() -> int:
    ...
    print(
        f"turn_step={args.turn_step_deg:.1f}deg move_step={args.move_step_m:.2f}m "
    ...
    try:
        ...
        for step in range(args.steps):
            ...
            # Only move_forward can fail due to a collision; feedback.collision is a
            # sticky whole-episode flag, so use feedback.success for this-step detection.
            step_collided = action is DiscreteAction.MOVE_FORWARD and not feedback.success
            ...
            if step_collided:
                ...
                recovery_action = (
                    ...
                    if last_turn is DiscreteAction.TURN_LEFT
                    else DiscreteAction.TURN_LEFT
                )
