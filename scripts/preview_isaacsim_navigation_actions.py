...
_KEY_ALIASES: dict[str, DiscreteAction] = {
    ...
    "s": DiscreteAction.STOP,
    " ": DiscreteAction.STOP,
    "up": DiscreteAction.LOOK_UP,
...
def _read_single_key() -> str | None:
    """Block for one raw keypress; arrow keys resolve to "up"/"down"/"left"/"right"."""
    fd = sys.stdin.fileno()
...
def _resolve_action(key: str) -> DiscreteAction | None:
    if key.isdigit() and int(key) in ACTION_BY_ID:
        return ACTION_BY_ID[int(key)]
    return _KEY_ALIASES.get(key.lower())
...
def main() -> int:
    ...
    for action_id, action in ACTION_BY_ID.items():
        print(f"  {action_id}: {action.value}")
    print("Aliases: w=move_forward  a/Left=turn_left  d/Right=turn_right")
    print("         space/s=stop    Up=look_up        Down=look_down")
    print("Press p or q to exit.\n")
    ...
    try:
        ...
        while True:
            ...
            action = _resolve_action(key)
            if action is None:
                print(f"Unrecognized key {key!r}; ignored.")
                continue
            ...
            print(
                f"[{frame_index:04d}] {action.value:<12s} "
                f"pos=({state.position[0]:+.2f},{state.position[1]:+.2f},{state.position[2]:+.2f}) "
                f"yaw={state.yaw:+.2f} success={feedback.success} {feedback.reason}"
            )
            if feedback.collision:
                print(f"Collision blocked: {feedback.reason}")
