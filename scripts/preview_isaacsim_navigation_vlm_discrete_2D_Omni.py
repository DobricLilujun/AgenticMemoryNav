#!/usr/bin/env python3
"""VLM-driven discrete-action navigation in Isaac Sim with a 2D top-down memory map.

This is a variant of ``preview_isaacsim_navigation_vlm_discrete.py`` that maintains
a live 2D top-down spatial memory map and feeds it to the VLM as an extra image at
each decision step. The map shows the robot pose, trajectory, and any semantic
landmarks reported by the VLM.

Run with Isaac Sim's bundled Python:

    conda deactivate
    ~/isaacsim/python.sh scripts/preview_isaacsim_navigation_vlm_discrete_2D_Omni.py \
        --config configs/isaacsim_realtime_agent_internscenes_test.yaml \
        --instruction "Find the curtain in the room" \
        --livestream \
        --public-ip 127.0.0.1
"""
...
from agentic_memory_nav.agent.vlm.discrete_navigation_2d_map import (  # noqa: E402
    VLMDiscreteNavigation2DMapAgent,
)
...
from agentic_memory_nav.visualization.topdown_map import create_topdown_map  # noqa: E402
...
def _build_agent(config, instruction: str, max_look_count: int) -> VLMDiscreteNavigation2DMapAgent:
    ...
    return VLMDiscreteNavigation2DMapAgent(
        instruction,
...
def main() -> int:
    ...
    map_dir = run.artifacts / "topdown_maps"
    ...
    topdown = create_topdown_map(
        width=20.0,
        height=20.0,
        resolution=0.05,
        robot_x=robot_start[0] if robot_start else 0.0,
        robot_y=robot_start[1] if robot_start else 0.0,
        robot_theta=math.radians(robot_yaw_deg),
    )

    agent = _build_agent(config, instruction, args.max_look_count)
    print(f"VLM discrete navigation + 2D map: instruction={instruction!r}, model={agent.model_id}")
    ...
    try:
        ...
        for step in range(args.steps):
            ...
            # Update the 2D map with the current robot pose before VLM decision.
            if frame.robot_pose is not None:
                topdown.update_robot_pose(
                    frame.robot_pose.position[0],
                    frame.robot_pose.position[1],
                    frame.robot_pose.yaw,
                )

            # Render and save the current top-down map.
            map_path = map_dir / f"topdown_{step:04d}.png"
            map_image = topdown.get_map_image()
            Image.fromarray(map_image).save(map_path)

            action, reason, confidence = agent.decide_action(frame)

            # Pass the rendered map to the agent so the next VLM call can see it.
            agent.set_map_image(map_image)

            # Heuristic: parse the VLM reason for any mentioned landmarks and add them.
            for lm in VLMDiscreteNavigation2DMapAgent.extract_landmarks_from_reason(reason):
                topdown.add_landmark_from_relative(
                    label=str(lm["label"]),
                    rel_distance=float(lm["distance_m"]),
                    rel_angle=float(lm["angle_rad"]),
                    category=str(lm["category"]),
                    color=str(lm["color"]),
                )
            ...
            if step_collided:
                ...
                continue
            # The episode runs for the configured number of steps; do not stop early
            # even if the VLM emits stop, so that the robot keeps approaching the target.
            time.sleep(1.0 / camera_fps)
    except KeyboardInterrupt:
        print("\nInterrupted by user")
    finally:
        run.write_json(
            "vlm_discrete_2d_omni_summary.json",
            {
        ...
        print(f"VLM discrete + 2D map artifacts: {run.path}", flush=True)
        run.close()
        executor.close()
    return 0
